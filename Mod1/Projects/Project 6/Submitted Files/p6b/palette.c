...
int get_palette(char palette_fname[], struct color pal[], int size){
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		sscanf(line,"%c %d %d %d %s \n",&pal[counter].code, &pal[counter].red_val, &pal[counter].green_val, &pal[counter].blue_val, pal[counter].name);
...
void show_palette(struct color pal[], int size){
	...
	for (i=0; i<size; i++){
      ...
      printf("pal[%d]:%c %d %d %d %s \n", i, pal[i].code, pal[i].red_val, pal[i].green_val, pal[i].blue_val, pal[i].name);
	}
	printf("\n");
}
...
struct color choose_closest(struct color pal[], int size, struct color orig_color){
	...
	closest_distance = sqrt(pow((orig_color.red_val - pal[0].red_val), 2) + pow((orig_color.green_val - pal[0].green_val), 2) + 
	...
	for (int i = 1; i < size; i++) {
		distance = sqrt(pow((orig_color.red_val - pal[i].red_val), 2) + pow((orig_color.green_val - pal[i].green_val), 2) + 
	...
	struct color closest_col = {.red_val = red_dist, .green_val = green_dist, .blue_val = blue_dist};
	return closest_col;
}
   ...
   fprintf(outfile, "P6\n %d %d 255\n", width, height);
	 ...
	 for (int i= 0; i < height*width; i++){
      ...
      fscanf(infile,"%c%c%c",&red_char, &green_char, &blue_char);
		...
		temp1.red_val = (int)red_char;
		temp1.green_val = (int)green_char;
		temp1.blue_val = (int)blue_char;
